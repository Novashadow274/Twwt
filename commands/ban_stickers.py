...
from telegram import Update from telegram.ext import ContextTypes from config import OWNER_ID, LOG_CHANNEL from logic import banned_stickers

async def banstk(update: Update, context: ContextTypes.DEFAULT_TYPE): """Ban a sticker set by name and log the action.""" user = update.effective_user if user.id != OWNER_ID: return
...
async def unbanstk(update: Update, context: ContextTypes.DEFAULT_TYPE): """Unban a sticker set by name and log the action.""" user = update.effective_user if user.id != OWNER_ID: return
