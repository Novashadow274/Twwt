...
def delete_and_log(context, chat_id, message_id, reason, user):
    """Delete a message and log the action to the log channel."""
    try:
        context.bot.delete_message(chat_id, message_id)
    except Exception:
        pass
    log_text = f"Deleted message from {user.mention_html()} in chat {chat_id}. Reason: {reason}"
    context.bot.send_message(LOG_CHANNEL, log_text, parse_mode='HTML')

def handle_message(update, context):
    """Generic message handler: filters banned content and spam."""
    ...
    for word in banned_words:
        if word.lower() in text.lower():
            delete_and_log(context, chat_id, msg.message_id, f"Banned word: {word}", user)
            return

    # Check links (simple regex)
    if re.search(r'http[s]?://', text):
        delete_and_log(context, chat_id, msg.message_id, "Link posted", user)
        return

    # Check spammy emoji (e.g., >10 emojis)
    if len(re.findall(r'[^\w\s,]', text)) > 10:
        delete_and_log(context, chat_id, msg.message_id, "Excessive emojis or symbols", user)
        return
